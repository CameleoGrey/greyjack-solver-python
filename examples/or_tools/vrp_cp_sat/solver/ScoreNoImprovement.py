...
class ScoreNoImprovement(cp_model.CpSolverSolutionCallback):
    """Print improving tuples and stop even if callbacks cease."""
    ...
    def __init__(
        ...
        self._medium_penalty = medium_penalty
    ...
    def start(self) -> None:
        ...
        self._thread = Thread(target=self._watch, name="vrp-idle-watchdog", daemon=True)
        self._thread.start()
    ...
    def record_improvement(self, score: tuple[int, int, int]) -> None:
        with self._lock:
    ...
    def on_solution_callback(self) -> None:
        self.record_improvement(
            (
                self.value(self._hard_penalty),
                self.value(self._medium_penalty),
                self.value(self._distance),
            )
        )
