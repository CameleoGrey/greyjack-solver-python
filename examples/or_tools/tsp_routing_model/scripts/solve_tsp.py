"""TSP file -> business domain -> RoutingModel cotwin -> reconstructed tour."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--input", type=Path, default=DEFAULT_INPUT, help="EUC_2D or EXPLICIT TSP file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--plot", action="store_true", help="Show the tour (requires matplotlib)"
    )
    args = parser.parse_args(argv)

    # Keep --help usable before installing the optional solver dependency.
    from examples.or_tools.tsp_routing_model.persistence.CotwinBuilder import (
        CotwinBuilder,
    )
    from examples.or_tools.tsp_routing_model.persistence.DomainBuilder import (
        DomainBuilder,
    )
    from examples.or_tools.tsp_routing_model.solver.TSPSolver import TSPSolver

    try:
        solver = TSPSolver(args.no_improvement_seconds, args.time_limit)
        builder = DomainBuilder(args.input)
        domain = builder.build_domain_from_scratch()
        print(
            f"Building RoutingModel: {len(domain.locations_list) - 1} stops",
            flush=True,
        )
