"""Generated domain -> Cross cotwin -> reconstructed facility metrics."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            f"Building Cross model: {len(domain.facilities)} facilities, "
        ...
        print(f"Cross iterations: {result.iterations}")
        print(f"Fixed-opening solves: {result.primal_solves}")
        print(f"Pricing solves: {result.pricing_solves}")
        print(f"Dual-master solves: {result.dual_master_solves}")
        print(f"Dual columns: {result.columns}")
