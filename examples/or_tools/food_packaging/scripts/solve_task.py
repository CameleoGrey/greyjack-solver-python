"""Generated food packaging -> CP-SAT cotwin -> replayed business schedule."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=float,
        default=180,
        help="Stop after this many seconds without a better schedule (default: 15)",
    )
    parser.add_argument("--time-limit", type=float, help="Optional total time limit")
    parser.add_argument(
        "--no-greedy-hints",
        action="store_true",
        help="Disable advisory initial schedule hints",
    )
    parser.add_argument(
        "--no-schedule",
        action="store_true",
