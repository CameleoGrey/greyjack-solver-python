...
class ScoreNoImprovement(cp_model.CpSolverSolutionCallback):
    ...
    def __init__(
        ...
        self._best_score: Optional[tuple[int, int]] = None
    ...
    def start(self) -> None:
        ...
        self._thread = Thread(
            target=self._watch, name="cloud-balancing-idle-watchdog", daemon=True
        )
        self._thread.start()
    ...
    def on_solution_callback(self) -> None:
        # Read integer components instead of the floating-point objective value.
        ...
        while not self._finished.wait(min(0.05, self._seconds)):
            with self._lock:
                if self._clock() >= self._deadline:
                    ...
                    # Retry until solve returns: a very short deadline may expire
                    # before CpSolver has installed its native solve wrapper.
