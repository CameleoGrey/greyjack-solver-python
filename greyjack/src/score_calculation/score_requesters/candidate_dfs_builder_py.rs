...
impl CandidateDfsBuilderPy {
    ...
    pub fn get_plain_candidate_dfs(
        &mut self,
        samples: Vec<Vec<f64>>,
    ) -> (HashMap<String, PyDataFrame>, HashMap<String, PyDataFrame>) {
        let (planning_entity_dfs, problem_fact_dfs) =
            self.candidate_dfs_builder.get_plain_candidate_dfs(&samples);
    ...
    pub fn get_incremental_candidate_dfs(
        ...
        HashMap<String, PyDataFrame>,
        HashMap<String, PyDataFrame>,
        HashMap<String, PyDataFrame>,
    ) {
        let (planning_entity_dfs, problem_fact_dfs, delta_dfs) = self
            .candidate_dfs_builder
            .get_incremental_candidate_dfs(&sample, &deltas);
