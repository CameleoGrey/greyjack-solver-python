...
impl VariablesManager {
    ...
    pub fn build_semantic_groups_dict(
        ...
        for i in 0..variables_vec.len() {
            ...
            for group_name in variable_semantic_groups {
                ...
                let ids = semantic_groups_dict.get_mut(group_name).unwrap();
                if !ids.contains(&i) {
                    ids.push(i);
                }
            }
    ...
    pub fn get_random_semantic_group_ids(&self) -> Result<(&Vec<usize>, &String), String> {
        if self.n_semantic_groups == 0 {
            return Err("No mutable semantic groups are available".to_string());
        }
        ...
        return Ok((group_ids, group_name));
    }

    pub fn get_column_random_value(&self, column_id: usize) -> f64 {
        let variable = &self.variables_vec[column_id];
        variable.sample()
    }
    ...
    pub fn fix_variables(&self, values_array: &mut Vec<f64>, ids_to_fix: Option<Vec<usize>>) {
        ...
        let stub_collection: () = range_ids
            .iter()
            .map(|i| values_array[*i] = self.variables_vec[*i].fix(values_array[*i]))
            .collect();
    }
