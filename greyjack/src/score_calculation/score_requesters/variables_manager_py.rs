...
impl VariablesManagerPy {
    ...
    pub fn new(variables_vec_py: Vec<GJPlanningVariablePy>) -> PyResult<Self> {
        let variables_vec: Vec<GJPlanningVariable> = variables_vec_py
            .iter()
            .map(|var_py| {
        ...
        for variable in &variables_vec {
            variable.validate().map_err(PyValueError::new_err)?;
        }
        ...
        let mut semantic_group_keys: Vec<String> = semantic_groups_dict.keys().cloned().collect();
        semantic_group_keys.sort();
    ...
    pub fn get_random_semantic_group_ids(&self) -> PyResult<(&Vec<usize>, &String)> {
        if self.n_semantic_groups == 0 {
            return Err(PyValueError::new_err(
                "No mutable semantic groups are available",
            ));
        }
        ...
        Ok((group_ids, group_name))
    }

    // FIXED: Use centralized RNG
    pub fn get_column_random_value(&self, column_id: usize) -> PyResult<f64> {
        let variable = self
            .variables_vec
            .get(column_id)
            .ok_or_else(|| PyIndexError::new_err("Variable index is out of range"))?;
        Ok(variable.sample())
    }
    ...
    pub fn fix_variables(
        &self,
        values_array: Vec<f64>,
        ids_to_fix: Option<Vec<usize>>,
    ) -> PyResult<Vec<f64>> {
        let range_ids;
        match ids_to_fix {
        ...
        let stub_collection: () = range_ids
            .iter()
            .map(|i| values_array_clone[*i] = self.variables_vec[*i].fix(values_array[*i]))
            .collect();
